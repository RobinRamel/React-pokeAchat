...
// to show color of the type in his badge
const TYPE_COLORS = {
  bug: 'B1C12E',
  dark: '4F3A2D',
  dragon: '755EDF',
...
export default class Pokemon extends Component {
  ...
  async componentDidMount() {
    ...
    //Here we need to transform height from Decimeters to meters
    const height = pokemonRes.data.height / 10;
    // Then weight from Hectograms to Kilograms
    ...
    .map( stat => {
      ...
      .toLowerCase()
      .split('-')
      .map(stat => stat.charAt(0).toUpperCase() + stat.substring(1))
      .join(' ');
    })
    .join(', ');

      // Get pokemon Description, catch rate, egggroup, gender ratio, hatch step
      ...
      await axios.get(pokemonSpeciesUrl).then( res => {
        ...
        //lets put the catchRate by % and not numerical number up to 255
